...
class proceso extends AW
{
    ...
    public function Listado()
    {
        ...
        $sql = "SELECT * FROM proceso {$sqlForm} ";
    ...
    {
        $sql = "select id from proceso where id='{$this->id}'";
        $res = $this->Query($sql);
    ...
    public function Actualizar()
    {
        ...
        if ($bResultado) {
            $sqlBitacora = "INSERT INTO `bitacora`
                                              ...
                                              VALUES
                                              ('0','PROCESO','ACTUALIZACION','(Nombre: {$this->nombre}°)',NULL,NULL,'{$this->user_id}',NOW())";
    ...
    public function Agregar()
    {
        ...
        if ($bResultado) {
            $sqlBitacora = "INSERT INTO `bitacora`
                                    (`id`,`modulo`,`operacion`,`modificacion`,`url_pdf`,`url_word`,`usuario`,`fecha`)
                                    VALUES
                                    ('0','PROCESO','AGREGADO','(Nombre: {$this->nombre}°)',NULL,NULL,'{$this->user_id}',NOW())";

            $this->NonQuery($sqlBitacora);

            $sql1 = "select id from proceso order by id desc limit 1";
            $res = $this->Query($sql1);
